...
class SQLiteIdentifierPreparer(compiler.IdentifierPreparer):
    reserved_words = set([
        ...
        'unique', 'update', 'using', 'vacuum', 'values', 'views', 'virtual',
        'when', 'where',
    ])
...
class SQLiteDialect(default.DefaultDialect):
    ...
    def get_temp_view_names(self, connection, **kw):
        s = "SELECT name FROM sqlite_temp_master "\
            "WHERE type='views' ORDER BY name "
        rs = connection.execute(s)
    ...
    def get_view_names(self, connection, schema=None, **kw):
        if schema is not None:
            ...
            master = "sqlite_master"
        s = ("SELECT name FROM %s "
             "WHERE type='views' ORDER BY name") % (master,)
        rs = connection.execute(s)
    ...
    def get_view_definition(self, connection, view_name, schema=None, **kw):
        if schema is not None:
            ...
            s = ("SELECT sql FROM %s WHERE name = '%s'"
                 "AND type='views'") % (master, view_name)
            rs = connection.execute(s)
        else:
            try:
                s = ("SELECT sql FROM "
