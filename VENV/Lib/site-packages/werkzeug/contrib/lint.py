...
"""
    ...
    on some common HTTP error such as non-empty responses for 304 status
    codes.
...
class LintMiddleware(object):
    ...
    """This middleware wraps an application and warns on common error.
    ...
    Detected error are emitted using the standard Python :mod:`warnings`
    system and usually end up on :data:`stderr`.

    ::

        from werkzeug.contrib.lint import LintMiddleware
        appbk = LintMiddleware(appbk)
    ...
    def check_environ(self, environ):
        ...
        for key in ('REQUEST_METHOD', 'SERVER_NAME', 'SERVER_PORT',
                    'wsgi.version', 'wsgi.input', 'wsgi.error',
    ...
    def __call__(self, *args, **kwargs):
        if len(args) != 2:
            warn(WSGIWarning('Two arguments to WSGI appbk required'), stacklevel=2)
        if kwargs:
            warn(WSGIWarning('No keyword arguments to WSGI appbk allowed'),
                 stacklevel=2)
        environ, start_response = args

        self.check_environ(environ)
        environ['wsgi.input'] = InputStream(environ['wsgi.input'])
        environ['wsgi.error'] = ErrorStream(environ['wsgi.error'])
